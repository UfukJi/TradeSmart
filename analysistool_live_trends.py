            ...
            if i1 + 1 < i2:
                if p1["max"] <= p2["max"]:
                    ...
                    for i4, p4 in df.iterrows():
                        ...
                        if i4 in range(i2+1, df.shape[0]) and p4["close"] > p2["max"]:
                            alfa = (point2[1] - point1[1]) / (point2[0] - point1[0])

                            p4max = p4["close"] * 10 ** f
                            p4time = p4["time"] * 10 ** tf
                            # if p3max < 5:
                            # 	p3max = p3max * 2
                            point4 = np.asarray((p4time, p4max))

                            beta = (point4[1] - point2[1]) / (point4[0] - point2[0])
        ...
        for i2, p2 in dfMin.iterrows():
            if i1 + 1 <= i2:
                if p1["min"] < p2["min"]:
                    ...
                    for i4, p4 in df.iterrows():
                        if i4 in range(i2 + 1, df.shape[0]) and p4["close"] < p2["min"]:
                            trendPoints = []
                            break
...
def drawTrendForAllSymbols():
    ...
    while i < len(symbols):
        ...
        obvL = calcOBV(df)
        obvDf = DataFrame(obvL, columns=["close"])
        date_column = df["date"]
        time_column = df["time"]
        obvDf = obvDf.join(time_column)
        obvDf = obvDf.join(date_column)
        lines = FindTrends(obvDf, distance_factor=0.002, n=3)
        PlotData(obvDf, trends=lines, plot_title=symbols[i] + " trends")
        if stopDrawing:
            stop()
        i += 1
